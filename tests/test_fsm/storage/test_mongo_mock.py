...
def _make_storage() -> MongoStorage:
    client_mock = MagicMock()
    storage = MongoStorage(client=client_mock)
    storage._collection = AsyncMock()
    return storage


class TestMongoStorageMock:
    ...
    def test_from_url(self):
        with patch("aiogram.fsm.storage.mongo.AsyncIOMotorClient") as mock_client:
    ...
    async def test_update_data_empty_result_deletes_doc(self):
        storage = _make_storage()
        storage._collection.find_one_and_update.return_value = {}
        result = await storage.update_data(key=STORAGE_KEY, data={"foo": "bar"})
        assert result == {}
